...
function turnon() {
    ...
    document.querySelectorAll(".trash").forEach(saveone => {
        saveone.addEventListener("click", (event) => {
            ...
            if (find == true) {
                ...
                loves.push(idsearch);
                window.localStorage.setItem("savelist", JSON.stringify(loves));
                saveone.setAttribute("src", "img/trash-bin.png");
            }
        });
    });
}
    ...
    if (loves.length == 0) {
        gamelist.insertAdjacentHTML("beforeend", `
                <div>
                    ...
                    <h1 class="text-title2 font-bold lg:text-3xl relative bottom-10">Your savelist is empty</h1>
                </div>
            `);
    } else {
            for (let check = 0; check < loves.length; check++) {
                ...
                if (loves[check] == datas.results[i].id) {
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <img id="${datas.results[i].id}" class="w-5 h-5 mr-3 cursor-pointer hover:scale-105 trash" src="img/trash-bin.png">
                    </div>
                </div>
            `);
            ...
            for (let check = 0; check < loves.length; check++) {
                if (loves[check] == datas.results[i].id) {
                    document.getElementById(`${datas.results[i].id}`).setAttribute("src", "img/trash-bin.png");
                }
            }
                }
                }
